...
def generate_password(self):#used to give a parameter
    """Generate a random password."""
    alphabet = string.ascii_letters + string.digits + string.punctuation#combination ie ascii letters + digits +specialchar
    password = ''.join(secrets.choice(alphabet) for _ in range(self))# this is used to pick random password . It is similar to random library
...
def main():
    ...
    parser.add_argument("-1", "--self", type=int,default=12)#this step is basically used for default value ie 12 and -l is aliasing and --self is used to start the char character with the int and other forms
    args = parser.parse_args()#here the value of 12 length is stored

    if args.self <= 0:#this condition will never be true as default value is 12 but incase the user use non ositive number then it will show an error
        print("Error: Length should be a positive integer.")
        return

    password = generate_password(args.self)# the return function stored here and then will print in next step
